...
std::vector<OptionHandler*> OptionHandlerFactory::createOptionHandlers()
{
  ...
  {
    OptionHandler* op(new UnitNumberOptionHandler(
        PREF_MIN_SPLIT_SIZE, TEXT_MIN_SPLIT_SIZE, "4M", 1_k, -1, 'k'));
    op->addTag(TAG_BASIC);
    op->addTag(TAG_FTP);
    op->addTag(TAG_HTTP);
    op->setInitialOption(true);
...
#define NULL_DEVICE "/dev/null"
#endif // !__MINGW32__
    OptionHandler* op(new LocalFilePathOptionHandler(
        PREF_RPC_STATIC_FILE, TEXT_RPC_STATIC_FILE, 
        /* defaultValue = */ NULL_DEVICE,
        /* acceptStdin = */ false,
        /* shortName = */ 0,
        /* mustExist = */ false,
        /* possibleValuesString = */ PATH_TO_DIR));
    op->addTag(TAG_RPC);
    handlers.push_back(op);
  }
  // HTTP/FTP options
  ...
  {
    OptionHandler* op(new UnitNumberOptionHandler(
        PREF_PIECE_LENGTH, TEXT_PIECE_LENGTH, "512k", 1_k, 1_g));
    op->addTag(TAG_ADVANCED);
    op->addTag(TAG_FTP);
    op->addTag(TAG_HTTP);
    op->setInitialOption(true);
